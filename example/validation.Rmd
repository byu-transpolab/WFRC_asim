...
oTAZ <- rep(1:2881, times = 1, each =2881) %>% 
  as.data.frame()
dist_v <- as.vector(dist_mtx$"/data/xydist")
## Add trip counts for each mode
DRIVEALONEFREE <- as.vector(t(pa_omx$"/data/DA"))
SHARED2FREE <- as.vector(t(pa_omx$"/data/SR2"))
SHARED3FREE <- as.vector(t(pa_omx$"/data/SR3p"))
Auto <- as.vector(t(pa_omx$"/data/auto"))
DRIVE_COM <- as.vector(t(pa_omx$"/data/dCRT"))
DRIVE_EXP <- as.vector(t(pa_omx$"/data/dEXP"))
DRIVE_LOC <- as.vector(t(pa_omx$"/data/dLCL"))
DRIVE_LRF <- as.vector(t(pa_omx$"/data/dLRT"))
DRIVE_HVY <- as.vector(t(pa_omx$"/data/dmode9"))
Motor <- as.vector(t(pa_omx$"/data/motor"))
Nonmotorized <- as.vector(t(pa_omx$"/data/nonmotor"))
Transit <- as.vector(t(pa_omx$"/data/transit"))
WALK_COM <- as.vector(t(pa_omx$"/data/wCRT"))
WALK_EXP <- as.vector(t(pa_omx$"/data/wEXP"))
WALK_LOC <- as.vector(t(pa_omx$"/data/wLCL"))
WALK_LRF <- as.vector(t(pa_omx$"/data/wLRT"))
WALK_HVY <- as.vector(t(pa_omx$"/data/wmode9"))
tlfd_wfrc <- data.frame(c(oTAZ, dTAZ, dist_v, DRIVEALONEFREE, SHARED2FREE, SHARED3FREE, Auto, DRIVE_COM, DRIVE_EXP,
                          DRIVE_HVY, DRIVE_LOC, DRIVE_LRF, Motor, Nonmotorized, Transit, WALK_COM, WALK_EXP, WALK_HVY,
                          WALK_LOC, WALK_LRF))
tlfd_wfrc <- matrix(c(oTAZ, dTAZ, dist_v, DRIVEALONEFREE, SHARED2FREE, SHARED3FREE, Auto, DRIVE_COM, DRIVE_EXP,
                          DRIVE_HVY, DRIVE_LOC, DRIVE_LRF, Motor, Nonmotorized, Transit, WALK_COM, WALK_EXP, WALK_HVY,
                          WALK_LOC, WALK_LRF), nrow = length(oTAZ))
```
